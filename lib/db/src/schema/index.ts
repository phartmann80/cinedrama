...
export const episodesTable = pgTable('episodes', {
  ...
  uniqueIndex('episodes_drama_ep_idx').on(table.dramaId, table.episodeNumber),
]);
...
export const unlocksTable = pgTable('unlocks', {
  ...
  episodeId: uuid('episode_id').notNull().references(() => episodesTable.id, { onDelete: 'cascade' }),
  createdAt: timestamp('created_at').notNull().defaultNow(),
}, (table) => [
  uniqueIndex('unlocks_user_episode_idx').on(table.userId, table.episodeId),
]);
...
export const coinTransactionsTable = pgTable('coin_transactions', {
  ...
  reason: text('reason').notNull(), // 'ad_reward' | 'episode_unlock' | 'purchase'
...
export const likesTable = pgTable('likes', {
  ...
  uniqueIndex('likes_user_episode_idx').on(table.userId, table.episodeId),
]);
...
export const coinTransactionsRelations = relations(coinTransactionsTable, ({ one }) => ({
  ...
  episode: one(episodesTable, { fields: [coinTransactionsTable.episodeId], references: [episodesTable.id] }),
}));
