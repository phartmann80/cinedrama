 ...
 * index — updating the videoUrl if it was previously null (migration-safe).
 *
 * The database always stores the real video URL for every episode.
 * Locking/redaction is enforced at the API layer based on the user's unlock status.
 */

import { drizzle } from 'drizzle-orm/node-postgres';
import pg from 'pg';
import * as schema from './schema/index.js';
import { dramasTable, episodesTable } from './schema/index.js';
...
async function seed() {
  ...
  for (const dramaData of DRAMAS) {
    // Upsert drama by title (idempotent)
    ...
    if (existing.length > 0) {
      ...
      console.log(`  ⏭  Drama already exists: "${dramaData.title}" (${dramaId})`);
    } else {
    ...
    // Upsert episodes — always store the real video URL.
    // ON CONFLICT updates videoUrl so existing rows with null URLs are repaired.
    // The API layer redacts the URL based on the user's unlock status, not the DB value.
    let upserted = 0;
    for (let ep = 1; ep <= dramaData.totalEpisodes; ep++) {
      ...
      await db
        ...
        .values({
          ...
          durationSeconds: 90 + ((ep * 7) % 61), // deterministic, no randomness
          videoUrl,
        ...
        .onConflictDoUpdate({
          ...
          set: {
            // Repair any previously-null videoUrl entries; leave other fields unchanged.
